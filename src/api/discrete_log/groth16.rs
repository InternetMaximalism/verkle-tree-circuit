...
use franklin_crypto::bellman::pairing::{CurveAffine, CurveProjective, Engine, Wnaf};

use franklin_crypto::bellman::pairing::ff::{Field, PrimeField};

use franklin_crypto::bellman::groth16::{Parameters, VerifyingKey};

use franklin_crypto::bellman::{
  Circuit, ConstraintSystem, Index, LinearCombination, SynthesisError, Variable,
};

use franklin_crypto::bellman::domain::{EvaluationDomain, Scalar};
  ...
  C: Circuit<E>,
{
  ...
  let mut h = vec![E::G1::zero(); powers_of_tau.as_ref().len() - 1];
  {
    ...
    println!("computing powers of tau...");
    ...
    println!("computing the H query with multiple threads...");
  ...
  println!("using inverse FFT to convert powers of tau to Lagrange coefficients...");
        ...
        .zip(ct.chunks(chunk))
      {
        ...
        scope.spawn(move |_| {
          for ((((((a, b_g1), b_g2), ext), at), bt), ct) in a
            ...
            .zip(ct.iter())
          {
            fn eval_at_tau<E: Engine>(powers_of_tau: &[Scalar<E>], p: &[(E::Fr, usize)]) -> E::Fr {
