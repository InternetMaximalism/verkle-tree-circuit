use franklin_crypto::bellman::plonk::commitments::transcript::{
  Blake2sTranscript, Prng, Transcript as RawTranscript,
};
    ...
    CS: ConstraintSystem<E>,
  {
    ...
    for i in 0..self.t {
      let inputs_i: AllocatedNum<E> =
        AllocatedNum::alloc(cs.namespace(|| "allocate inputs[i]"), || {
      ...
      let output_i = inputs_i.add_constant(cs.namespace(|| "add inputs_i to c"), c)?;
...
impl<E: JubjubEngine> MixCircuit<E> {
    ...
    CS: ConstraintSystem<E>,
  {
    ...
    for i in 0..self.t {
      let mut lc = AllocatedNum::zero(cs.namespace(|| "allocate zero"))?;
      for j in 0..self.t {
        let reader = &mut std::io::Cursor::new(hex::decode(&M[j][i][2..]).unwrap());
        let m = read_point_be::<E::Fr>(reader).unwrap();
        let wrapped_m = AllocatedNum::alloc(cs.namespace(|| "allocate M"), || Ok(m))?;
        let inputs_j = AllocatedNum::alloc(cs.namespace(|| "allocate inputs[j]"), || {
          Ok(self.inputs[j].unwrap())
          // self.inputs[j].ok_or(SynthesisError::UnconstrainedVariable)
        })?; // TODO: uncheck
        let tmp = wrapped_m.mul(cs.namespace(|| "multiply wrapped_m by inputs_j"), &inputs_j)?;
        lc = lc.add(cs.namespace(|| "add lc to tmp"), &tmp)?;
      }
...
impl<E: JubjubEngine> PoseidonCircuit<E> {
    ...
    CS: ConstraintSystem<E>,
  {
    ...
    for i in 0..(N_ROUNDS_F + N_ROUNDS_P - 1) {
      ...
      for j in 0..self.t {
        ark[i].inputs[j] = if i == 0 {
          ...
          let input_j = AllocatedNum::alloc(cs.namespace(|| "allocate self.inputs[j]"), || {
            Ok(self.inputs[j].unwrap())
            // self.inputs[j].ok_or(SynthesisError::UnconstrainedVariable)
          })?;
          let zero = AllocatedNum::zero(cs.namespace(|| "allocate zero"))?;
          let input_j = AllocatedNum::conditionally_select(
            cs.namespace(|| "select self.input[j] if j < input_num"),
            &input_j,
            &zero,
          ...
          let input_j =
            AllocatedNum::alloc(cs.namespace(|| "allocate last_mix.outputs[j]"), || {
      ...
      if i < N_ROUNDS_F / 2 || i >= N_ROUNDS_P + N_ROUNDS_F / 2 {
        ...
        for j in 0..self.t {
          sigma_f[k].push(SigmaCircuit::default());
          sigma_f[k][j].input = ark[i].outputs[j];
          sigma_f[k][j].run(cs.namespace(|| "sigma_f_k_j"))?;
          last_mix.inputs[j] = sigma_f[k][j].output;
        }
      } else {
        let k = i - N_ROUNDS_F / 2;
        sigma_p.push(SigmaCircuit::default());
        sigma_p[k].input = ark[i].outputs[0];
        sigma_p[k].run(cs.namespace(|| "sigma_p_k"))?;
        last_mix.inputs[0] = sigma_p[k].output;
        for j in 1..T {
          last_mix.inputs[j] = ark[i].outputs[j];
        }
      }
      last_mix.run(cs.namespace(|| "mix_i"))?;
    }
    ...
    let output_0 = AllocatedNum::alloc(cs.namespace(|| "allocate last_mix.outputs[0]"), || {
