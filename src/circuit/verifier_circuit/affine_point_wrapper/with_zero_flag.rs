use franklin_crypto::bellman::pairing::ff::{BitIterator, Field};
...
//   bigint::field::{FieldElement, RnsParameters},
//   boolean::Boolean,
//   curve::sw_affine::AffinePoint,
//   verifier_circuit::affine_point_wrapper::{aux_data::AuxData, WrappedAffinePoint},
// };
...
fn add<'b, E: Engine, CS: ConstraintSystem<E>>(
  ...
  (
    FieldElement<'b, E, <E::G1Affine as CurveAffine>::Base>,
    FieldElement<'b, E, <E::G1Affine as CurveAffine>::Base>,
  ),
  SynthesisError,
> {
...
impl<'a, E: Engine> WrappedAffinePoint<'a, E> for WrapperWithFlag<'a, E> {
  ...
  fn alloc<CS: ConstraintSystem<E>, AD: AuxData<E>>(
    ...
    let res = WrapperWithFlag { is_zero, point };
  ...
  fn subgroup_check<CS: ConstraintSystem<E>, AD: AuxData<E>>(
    ...
    for i in BitIterator::new(exp) {
      if found_one {
        if i {
          // calcilate [2]P_2
          let (P_2_doubled_x, P_2_doubled_z) =
            double(cs, P_2_x.clone(), P_2_z.clone(), params, &b)?;
          // calculate P_1 ⊕ P_2
          let (P_1_2_x, P_1_2_z) = add(
            cs,
            P_1_x,
            P_1_z,
            P_2_x,
            P_2_z,
            &mut self.point.x.clone(),
            params,
            &b,
          )?;
          ...
          // calculate [2]P_1
          let (P_1_doubled_x, P_1_doubled_z) =
            double(cs, P_1_x.clone(), P_1_z.clone(), params, &b)?;
          // calculate P_1 ⊕ P_2
          let (P_1_2_x, P_1_2_z) = add(
            cs,
            P_1_x,
  ...
  fn multiexp<CS: ConstraintSystem<E>, AD: AuxData<E>>(
    cs: &mut CS,
...
impl<'a, E: Engine> WrapperWithFlag<'a, E> {
  fn multiexp_naive<CS: ConstraintSystem<E>, AD: AuxData<E>>(
    ...
    let mut acc = points[0]
      .clone()
      .mul(cs, &scalars[0], bit_limit, params, aux_data)?;

    for (point, scalar) in points.iter().zip(scalars).skip(1) {
      let mut temp = point
        .clone()
        .mul(cs, &scalar, bit_limit, params, aux_data)?;
  ...
  fn mul_advanced<CS: ConstraintSystem<E>>(
    ...
    let zero = AffinePoint::zero(params);
    let mut res_point =
      AffinePoint::select(cs, &acc_equals_offset_flag, zero.clone(), acc_minus_offset)?;
